...
function buildPixPayload({ key, keyType, name, city, amount, txid, }) {
    const fmt = (id, val) => {
        ...
        <main className="flex-1 mx-auto max-w-lg px-6 py-12 w-full text-center">
          <div className="rounded-2xl border border-border bg-background p-8 space-y-6">
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <img src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(buildPixPayload({
            ...
            }))}`} alt="QR Code PIX" className="rounded-xl border border-border" width={200} height={200}/>
            </div>

            <div className="space-y-2">
              <p className="text-sm font-medium">Ou copie o código PIX:</p>
              <div className="flex gap-2">
                <input readOnly value={buildPixPayload({
                key: pixSettings?.pix_key ?? '46960905000104',
                keyType: pixSettings?.pix_key_type ?? 'cnpj',
                name: pixSettings?.pix_beneficiary ?? 'ELMADAN QUEIROZ SILVEIRA BENITES',
                city: pixSettings?.pix_city ?? 'Rondonopolis',
            ...
            <div className="space-y-3">
              <a href={`https://wa.me/${pixSettings?.whatsapp_number ?? '5566984266994'}?text=${encodeURIComponent(`Olá! Acabei de fazer o pedido #${orderId?.slice(0, 8)} no site e realizei o pagamento do PIX de R$ ${amountDue.toFixed(2).replace('.', ',')}. Pode confirmar?`)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full rounded-full bg-[#25d366] text-white py-3 text-sm font-medium hover:opacity-90 transition-opacity" onClick={() => { clear(); }}>
                <MessageCircle className="size-4"/>
                Confirmar pagamento pelo WhatsApp
              </a>
              <Button variant="ghost" className="w-full text-sm" onClick={() => { clear(); navigate({ to: '/conta' }); }}>
                Ver meus pedidos
              </Button>
            </div>
          </div>
      ...
      <main className="flex-1 mx-auto max-w-6xl px-6 py-12 w-full">
        ...
        <div className="grid lg:grid-cols-[1fr_400px] gap-12">
          <div className="space-y-4">
            {items.map((it) => (<div key={it.id} className="flex gap-4 p-4 border border-border rounded-xl">
