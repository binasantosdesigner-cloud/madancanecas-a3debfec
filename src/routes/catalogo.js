        ...
        <section className="px-6 mb-12">
          <div className="mx-auto max-w-7xl">
            ...
            : "bg-transparent border-border text-muted-foreground hover:border-brand-gold hover:text-brand-gold")}>
              ...
              {categories.map((cat) => (<button key={cat.id} onClick={() => handleCategoryClick(cat.slug)} className={cn("px-5 py-2.5 rounded-full text-sm font-medium transition-all border shrink-0", activeCategory === cat.name
                ? "bg-brand-gold text-brand-cream border-brand-gold shadow-sm"
                : "bg-transparent border-border text-muted-foreground hover:border-brand-gold hover:text-brand-gold")}>
                  {cat.name}
                </button>))}
