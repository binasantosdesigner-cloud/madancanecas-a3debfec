...
import { User, Package, CreditCard, ShoppingCart, Heart, Palette, MapPin, Bell, Award, HelpCircle, Settings, LogOut, LayoutDashboard, Camera, Plus, Pencil, Trash2, Check, Brush, ArrowRight, CheckCircle2, Clock } from "lucide-react";
...
function AccountPage() {
    ...
    useEffect(() => { if (!loading && !user)
        navigate({ to: "/login" }); }, [user, loading, navigate]);
    if (!user)
        return null;
      ...
      <main className="flex-1 mx-auto w-full max-w-7xl px-4 sm:px-6 py-8 lg:py-12">
        ...
        <nav className="lg:hidden -mx-4 sm:mx-0 mb-6 overflow-x-auto">
          <div className="flex gap-2 px-4 sm:px-0">
            {MENU.map((m) => (<button key={m.id} onClick={() => setSection(m.id)} className={`shrink-0 flex items-center gap-2 px-4 py-2 rounded-full text-sm border transition ${section === m.id
        ...
        <div className="grid lg:grid-cols-[260px_minmax(0,1fr)] gap-8">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24 rounded-2xl border border-border bg-card p-2">
              {isAdmin && (<div className="mb-4 pb-4 border-b border-border/60">
                  <Link to="/admin" className="flex items-center gap-3 w-full rounded-xl px-4 py-3 text-sm font-medium bg-accent/10 text-accent hover:bg-accent/20 transition-colors">
                    <LayoutDashboard className="size-4 shrink-0"/>
                    Painel Administrativo
                  </Link>
                </div>)}
              {MENU.map((m) => (<button key={m.id} onClick={() => setSection(m.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition ${section === m.id
                ? "bg-accent/10 text-accent font-medium"
                : "text-foreground/75 hover:bg-muted/60"}`}>
                  <m.icon className="h-4 w-4 shrink-0"/>
                  <span className="truncate">{m.label}</span>
                </button>))}
              <button onClick={async () => { await signOut(); navigate({ to: "/" }); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-foreground/75 hover:bg-muted/60 mt-1">
...
/* ---------------- Header ---------------- */
function AccountHeader({ onSignOut }) {
    const { user } = useAuth();
    ...
    return (<header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 mb-8">
...
function DashboardSection({ onNavigate }) {
    ...
    const total = orders?.reduce((s, o) => s + Number(o.total || 0), 0) ?? 0;
    const cards = [
    ...
    return (<div className="space-y-8">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((c) => (<button key={c.label} onClick={c.onClick} className="text-left rounded-2xl border border-border bg-card p-5 hover:border-accent/50 transition group">
      ...
      {pendingArts.length > 0 && (<button onClick={() => onNavigate("aprovacao")} className="w-full text-left rounded-2xl border border-accent/40 bg-accent/5 p-6 hover:bg-accent/10 transition grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4">
      ...
      <SectionCard title="Pedidos recentes" action={<button onClick={() => onNavigate("pedidos")} className="text-sm text-accent hover:underline">Ver todos</button>}>
        {(orders?.length ?? 0) === 0 ? (<EmptyState text="Você ainda não fez nenhum pedido." ctaTo="/produtos" ctaText="Ver produtos"/>) : (<div className="space-y-3">
...
function PerfilSection() {
    ...
    return (<SectionCard title="Meu Perfil">
      <div className="flex flex-col sm:flex-row sm:items-center gap-6 pb-6 border-b border-border">
        <div className="relative w-fit">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="absolute -bottom-1 -right-1 h-9 w-9 rounded-full bg-accent text-accent-foreground grid place-items-center shadow hover:opacity-90" aria-label="Alterar foto">
            <Camera className="h-4 w-4"/>
          </button>
          <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f)
        onUpload(f); }}/>
        </div>
        <div className="min-w-0">
      ...
      <form className="grid sm:grid-cols-2 gap-4 pt-6" onSubmit={(e) => { e.preventDefault(); save.mutate(); }}>
...
function PedidosSection() {
    ...
    if (isLoading)
        return <SkeletonCard />;
    return (<SectionCard title="Meus Pedidos">
      {(orders?.length ?? 0) === 0 ? (<EmptyState text="Você ainda não fez nenhum pedido." ctaTo="/produtos" ctaText="Ver produtos"/>) : (<div className="space-y-4">{orders.map((o) => <OrderRow key={o.id} order={o}/>)}</div>)}
    </SectionCard>);
}
const TIMELINE = ["pending", "in_production", "shipped", "completed"];
function OrderRow({ order, compact }) {
    const idx = TIMELINE.indexOf(order.status);
    return (<div className="p-5 border border-border rounded-xl bg-background">
      ...
      <div className="mt-3 flex items-center gap-2">
        {order.payment_status === 'paid' ? (<span className="inline-flex items-center gap-1.5 text-xs text-green-700 bg-green-100 px-2.5 py-1 rounded-full font-medium">
            <CheckCircle2 className="size-3"/> Pagamento confirmado
          </span>) : (<span className="inline-flex items-center gap-1.5 text-xs text-amber-700 bg-amber-100 px-2.5 py-1 rounded-full font-medium">
            <Clock className="size-3"/> Aguardando confirmação do sinal
          </span>)}
      </div>

      {!compact && (<div className="mt-4 text-sm text-muted-foreground space-y-0.5">
          {order.items.map((i, k) => <div key={k}>{i.quantity}x {i.title}</div>)}
        </div>)}
      {!compact && idx >= 0 && order.status !== "cancelled" && (<div className="mt-5 flex items-center gap-1">
          {TIMELINE.map((s, i) => (<div key={s} className="flex-1 flex items-center gap-1">
...
function PagamentosSection({ onNavigate }) {
    const { user } = useAuth();
    const { data: orders, isLoading } = useQuery({
        enabled: !!user,
        queryKey: ['orders', user?.id],
    ...
    return (<SectionCard title="Pagamentos">
      {pending.length > 0 && (<div className="mb-6">
          <h3 className="text-sm font-semibold mb-3 text-amber-700">Aguardando confirmação</h3>
          <div className="space-y-3">
            {pending.map((o) => (<div key={o.id} className="p-4 rounded-xl border border-amber-200 bg-amber-50">
      ...
      {paid.length > 0 && (<div>
          ...
          <div className="space-y-3">
            {paid.map((o) => (<div key={o.id} className="p-4 rounded-xl border border-green-200 bg-green-50">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-xs text-muted-foreground">Pedido #{o.id.slice(0, 8)}</p>
      ...
      {(orders?.length ?? 0) === 0 && (<EmptyState text="Nenhum pagamento ainda." ctaTo="/catalogo" ctaText="Ver produtos"/>)}
    </SectionCard>);
}
const blankAddress = {
...
function EnderecosSection() {
    ...
    const remove = useMutation({
        mutationFn: async (id) => { const { error } = await supabase.from("addresses").delete().eq("id", id); if (error)
            throw error; },
    ...
    return (<SectionCard title="Endereços" action={<Button size="sm" onClick={() => { setEditing(null); setOpen(true); }}>
          <Plus className="h-4 w-4 mr-1"/> Novo
        </Button>}>
      {(addresses?.length ?? 0) === 0 ? (<EmptyState text="Você ainda não cadastrou endereços."/>) : (<div className="grid sm:grid-cols-2 gap-4">
          {addresses.map((a) => (<div key={a.id} className="p-4 border border-border rounded-xl bg-background">
              ...
              {!a.is_default && (<button onClick={() => setDefault.mutate(a.id)} className="text-xs text-accent hover:underline mt-3">
                  Definir como padrão
                </button>)}
            </div>))}
        </div>)}
...
function AjudaSection() {
    ...
    return (<SectionCard title="Central de Ajuda">
      <div className="grid sm:grid-cols-2 gap-3">
        {links.map((l) => (<Link key={l.to} to={l.to} className="p-4 rounded-xl border border-border bg-background hover:border-accent/50 transition flex items-center justify-between gap-3">
...
function SectionCard({ title, action, children }) {
    return (<div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
...
function EmptyState({ text, ctaTo, ctaText }) {
    return (<div className="text-center py-10 border border-dashed border-border rounded-xl">
      <p className="text-muted-foreground">{text}</p>
      {ctaTo && ctaText && (<Link to={ctaTo} className="inline-block mt-4"><Button>{ctaText}</Button></Link>)}
    </div>);
}
...
function PlaceholderLink({ to, title, desc, cta }) {
    return (<SectionCard title={title}>
      <div className="py-10 text-center">
        <p className="text-muted-foreground max-w-md mx-auto">{desc}</p>
...
function SkeletonCard() {
    return (<div className="rounded-2xl border border-border bg-card p-8 space-y-3">
    ...
    const { data: favProducts, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const { data: favs } = await supabase
                ...
                .eq("user_id", user.id);
            return (favs ?? []).map((f) => f.products).filter(Boolean);
        },
    });
    if (isLoading)
        return <SkeletonCard />;
    return (<SectionCard title="Meus Favoritos">
      {(favProducts?.length ?? 0) === 0 ? (<EmptyState text="Você ainda não favoritou nenhum produto." ctaTo="/catalogo" ctaText="Explorar catálogo"/>) : (<div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {favProducts.map((p) => (<div key={p.id} className="relative group">
              <Link to="/produto/$slug" params={{ slug: p.slug }} className="block rounded-xl overflow-hidden bg-secondary aspect-square">
                {p.image_url ? (<img src={p.image_url} alt={p.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>) : (<div className="w-full h-full flex items-center justify-center text-[10px] uppercase tracking-widest text-muted-foreground/40">Madan</div>)}
              </Link>
              <button onClick={() => toggle(p.id)} className="absolute top-2 right-2 size-7 rounded-full bg-white/90 flex items-center justify-center text-[#e8509a]" aria-label="Remover dos favoritos">
                <Heart className="size-3.5 fill-[#e8509a]"/>
              </button>
              <div className="mt-2">
                <Link to="/produto/$slug" params={{ slug: p.slug }} className="text-sm font-medium hover:text-[#e8509a] transition-colors line-clamp-1">
                  {p.title}
                </Link>
